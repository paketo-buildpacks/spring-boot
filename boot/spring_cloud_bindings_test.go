...
func testSpringCloudBindings(t *testing.T, context spec.G, it spec.S) {
	...
	it.Before(func() {
		...
		ctx.Layers.Path, err = os.MkdirTemp("", "spring-cloud-bindings-layers")
		Expect(err).NotTo(HaveOccurred())

		ctx.Application.Path, err = os.MkdirTemp("", "spring-cloud-bindings-app")
		Expect(err).NotTo(HaveOccurred())
	})
	...
	it("contributes Spring Cloud Bindings", func() {
		...
		s, _ := boot.NewSpringCloudBindings(filepath.Join(ctx.Application.Path, "test-lib"), dep, dc)
