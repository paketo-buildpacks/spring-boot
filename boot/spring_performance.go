...
func (s SpringPerformance) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	...
	layer, err := s.LayerContributor.Contribute(layer, func() (libcnb.Layer, error) {
		...
		if jreVersion >= 25 {
			...
			trainingRunArgs = append(trainingRunArgs, "-XX:AOTCacheOutput=application.aot -Xlog:cds")
		} else {
			trainingRunArgs = append(trainingRunArgs, "-XX:ArchiveClassesAtExit=application.jsa -Xlog:cds")
		}
